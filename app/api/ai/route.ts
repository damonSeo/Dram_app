...
블로그 포스트 구성 — 헤드라인(대괄호 제목)은 [외관] [향] [맛] [여운] 단 4개만 사용합니다.
나머지(도입·증류소·보틀 설명·총평)는 헤드라인 없이 자연스럽게 이어지는 서술형 문단으로 작성하세요.

1) 도입 문단 (헤드라인 없음)
이 위스키와의 첫인상으로 시작해, 증류소 이야기와 이 보틀의 의미를 하나의 자연스러운 서술로 풀어내세요. 끊긴 항목 나열이 아니라 흐르는 글이어야 합니다. 6~9문장.
- 증류소: ${distInfo
  ? '위 증류소 참고 자료를 바탕으로 역사·지역·스타일·특징을 글 속에 녹여 서술 (자료를 그대로 베끼지 말 것). 이 보틀이 라인업에서 차지하는 위치도 자연스럽게 언급.'
  : `${brand} 증류소에 대해 알려진 일반적인 정보(지역·스타일·특징)를 서술형으로 간략히. 불확실하면 일반론으로.`}
- 보틀 설명: 숙성·캐스크·도수·빈티지가 풍미에 어떤 의미인지 입문자도 이해하도록 글의 흐름 안에서 쉽게 풀어 설명.

[외관]
잔에 담긴 색상과 점도를 감각적으로 묘사하는 서술형 문단. 3~4문장.

[향]
첫 잔을 들었을 때 퍼지는 향의 레이어를 순서대로 묘사. 구체적인 비유 포함. 서술형 5~6문장.

[맛]
입 안에서 전개되는 맛의 변화와 질감, 음식 페어링 힌트. 서술형 5~6문장.

[여운]
마신 후 남는 여운의 길이와 특성. 서술형 3~4문장.

(이후 헤드라인 없이 마무리 서술 문단)
${userComment ? `작성자 본인의 시음 코멘트("${userComment}")를 글의 흐름 속에 자연스럽게 녹이고, ` : ''}${overallGuide.replace(/^/, '')}
위 내용을 별도 제목 없이 하나의 마무리 서술 문단으로 자연스럽게 이어 작성하세요. 5~7문장.

마지막 줄에 관련 해시태그 8~10개 (한 줄, 헤드라인 없이).

작성 기준:
- 전체 1,400~1,800자 분량
- 서술형(에세이체)으로 매끄럽게 이어지는 글 — 항목 나열·불릿 금지
- 대괄호 제목은 오직 [외관] [향] [맛] [여운] 4개만, 그 외 어떤 제목·소제목·번호도 쓰지 말 것
- 반드시 순수한 한국어 (영어 단어·로마자 절대 금지 — Nose → 향, Palate → 맛, Finish → 여운, Cask → 캐스크, Color → 색상, Overall → 총평)
- 마크다운 헤더(#)·불릿 기호 금지
- 본문만 출력`
    }
